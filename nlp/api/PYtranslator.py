...
class Translator(Exception):
    ...
    async def infer_msg(self, tts, rsp=None):
        cause = "Unknown"
...
class google_translator:
    ...
    def __init__(self, url_suffix="com.np", timeout=10, proxies=None):
    ...
    async def _package_rpc(self, text, lang_src='auto', lang_tgt='auto'):
    ...
    async def translate(self, text, lang_tgt='auto', lang_src='auto', pronounce=False):
        ...
        freq = await self._package_rpc(text, lang_src, lang_tgt)
    ...
    async def detect(self, text):
        ...
        freq = await self._package_rpc(text)
        response = requests.Request(method='POST',
                                    url=self.url,
                                    data=freq,
                                    headers=headers)
